...
$tasks = @{
    ...
    "low_pass_filter" = "Write a function that applies a low-pass Butterworth filter to a signal. Parameters: signal : numpy.ndarray - Input signal array, fs : float - Sampling frequency in Hz, cutoff_freq : float - Cutoff frequency in Hz, order : int - Filter order. Returns: numpy.ndarray - Filtered signal. Use scipy.signal library."
}
